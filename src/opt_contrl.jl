...
function solve_gen_optimal_control(OCP::OptControlProb, path::Vector{Int64}, gen_MILP::Vector{Bool}, N::Int64; linear = true)
    ...
    g_min,g_max = 0, 100     #Bounds on the States
    b_min, b_max = 6, 99.9
    
    u_min, u_max = 0, 100     # Bounds on Control
    ...
    for timei in 2:N
        if linear
            @constraint(myModel, g[timei] == g[timei-1] - u[timei]/100*Zvec[timei-1])
            ...
            @NLconstraint(myModel, g[timei] == g[timei-1] - mdot_normed(u[timei]/100, Zvec[timei-1]) )
            # @NLconstraint(myModel,   g[timei] == g[timei-1] - u[timei]/100*Zvec[timei-1])
        ...
        @constraint(myModel, u[timei] <= 100*noise_i)
